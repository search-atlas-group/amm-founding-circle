...
Idempotent: an entry whose command string matches is replaced, not duplicated. Every other
hook in the file is preserved byte-for-byte in content. Writes a timestamped .bak first and
validates the result parses as JSON before replacing the original.
"""
...
def main():
    ...
    if len(args) < 3:
        print("usage: merge_settings.py <settings.json> <event> <command> [--timeout N] [--remove]", file=sys.stderr)
        return 2
