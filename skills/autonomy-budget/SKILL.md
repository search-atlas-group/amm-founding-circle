...
- **Claude Code's native Workflow tool** — the built-in `agent()` / `pipeline()` / `parallel()` orchestration primitive fans out and stages dozens of agents inside one run, with worktree isolation for conflicting edits. It's bounded to that one run, though — no time cap across days, no never-touch walls, no crash-resume. Use it as the fan-out engine *inside* a budgeted run; use `autonomy-budget` for the rails that let the run survive unattended across days.
