...
| [model-gear-router](model-gear-router/SKILL.md) | Decides which provider and model runs each task from an orchestrator layer, using the tier/mission split, a job-class routing table, the same-family review ban, and a precedence order that keeps your explicit choice on top. | One model is doing five jobs; an expensive model is burning on cheap work; fanning work out to several models in parallel. | Your provider list + job class → a filled gear table + per-task routing + parallel fan-out rules. | L6 |
...
| [team-accountability-chart](team-accountability-chart/SKILL.md) | Maps the agency by seat/function (EOS Accountability Chart), then generates two synced views: a complete internal one and a trimmed client-facing one (point-of-contact only, no comp/gaps). | "Who owns this" keeps stalling; building one system that's genuinely different for employees vs. clients without leaking internal org info. | Seat list + owners + responsibilities → internal chart + filtered client-facing chart, regenerated from one source. | L4 |
| [agency-scorecard](agency-scorecard/SKILL.md) | Builds and runs an EOS-style weekly Scorecard — 5–15 leading numbers pulled automatically from wherever they live, with off-track ones flagged before the weekly meeting. | You only check numbers monthly (too late to act); want the Scorecard ready before L10 instead of built during it. | Metric list + goal ranges + sources → weekly Scorecard table with trend + flags. | L5 |
| [division-operating-model](division-operating-model/SKILL.md) | Keeps a multi-person team's platform usage from splintering: mandatory vendor training as staff continuing education, a weekly team + monthly division rollout meeting, vendor playbooks rewritten margin-safe for what you actually sell, and new tactics tested on low-risk clients first. | Team platform usage has splintered by person; vendor updates aren't reliably reaching staff; tool spend keeps creeping ahead of delivered margin. | Vendor update stream + staff roster → training completion tracking + rollout decisions + margin-scoped internal playbooks. | L5 |
| [transparent-guarantee-design](transparent-guarantee-design/SKILL.md) | Builds a sales guarantee that closes deals without exposing the agency to a metric it can't hit: one self-contained measurable outcome, a defined tracking mechanism before it ships, a step-function path to bigger promises, and a concrete fallback (fee reduction) if it's missed. | Close rate is stalling on trust; an existing guarantee is vague enough that "did we hit it" is an argument, not a fact; a guarantee promises revenue/ROI the agency doesn't fully control. | Service + target metric → tracked guarantee spec (metric, tracking mechanism, timeline, fallback). | L4 |
| [quarterly-rocks-tracker](quarterly-rocks-tracker/SKILL.md) | Tracks EOS-style quarterly "Rocks" with weekly on-track/at-risk/off-track pulses and a binary done/not-done verdict at quarter close. | Quarterly goals quietly die by week 6; want the Rocks section of L10 prepped before you walk in. | Rocks list (owner, measure, due date) → weekly pulse + quarter-close verdict. | L5 |
| [client-health-score](client-health-score/SKILL.md) | Rolls engagement, deliverable timeliness, and sentiment into one internal early-warning score per client, catching churn risk months before a renewal or cancellation. | A cancellation "came out of nowhere"; want an honest read on shaky accounts before the renewal push. | Reply time + report engagement + payment + deliverables + sentiment → weekly per-client score + trend + flags. | L6 |
