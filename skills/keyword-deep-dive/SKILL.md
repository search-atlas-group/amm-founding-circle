...
description: Use when planning to rank for a specific keyword. The agent Googles it, reads the top 10, classifies intent, reads the top 3 competitor pages, and produces a 90-day ranking plan with intent, SERP analysis, and content recommendations. When a SearchAtlas MCP is connected, leverages SA tools (rank tracking, brand vault, GBP, OTTO, LLM Visibility) first before falling back to generic web search.
---
...
SearchAtlas's keyword research suite gives this skill 100x the data of a generic web search. Always start with the Atlas tools.

| Phase | SA MCP tool | What it gives you |
|---|---|---|
| Core | `keyword_research` → `lookup_keyword` | Search volume, CPC, competition, intent, trend. Single source of truth. |
| Core | `keyword_research` → `get_keyword_intent` | Atlas's intent classifier — informational / commercial / transactional / navigational / local. |
| SERP | `analysis` → `get_serp_overview` | Top 10 URLs, page types, SERP features — what's actually ranking now. |
| Variations | `keyword_research` → `lookup_keyword` with related/long-tail flags | Related keywords, long-tail variants, question forms. |
| Competitive | `organic` → `get_organic_competitors` | Who's already winning the keyword. Brand mix tells you whether it's contestable. |
| Coverage | `organic` → `get_organic_keywords(project_identifier=<domain>)` | How the brand currently performs for the keyword cluster. Identifies cannibalization. |
